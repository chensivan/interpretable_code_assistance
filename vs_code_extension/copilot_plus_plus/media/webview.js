...
document.getElementById("reload").addEventListener("click", function () {
  ...
  if (mode == iconDic["icon-group"]) {
    ...
    if (document.getElementById("submitTool-group")) {
      document
        .getElementById("submitTool-group")
        .parentNode.removeChild(document.getElementById("submitTool-group"));
    }
    if (document.getElementById("editTool-group")) {
      document
        .getElementById("editTool-group")
        .parentNode.removeChild(document.getElementById("editTool-group"));
    }
  } else {
    reloadSidePanel();
  }
...
document.getElementById("help").addEventListener("mouseover", function (event) {
  if (!document.getElementById("help-text")) {
    ...
    div.style =
      ...
      y +
      "px; background: white; border: 1px solid black; padding: 10px; z-index: 9999;";
        ...
        if (document.getElementById("submitTool-group")) {
          document
            ...
            .parentNode.removeChild(
              document.getElementById("submitTool-group")
            );
        }
        if (document.getElementById("editTool-group")) {
          document
...
function logSelectedElement(elmntRid, ele) {
  getLogByRID(USERID, elmntRid).then((data) => {
    if (data.length > 0) {
      createHistoryPanel([data[0]], true, elmntRid);
...
document.addEventListener("click", function (event) {
  if (
    ...
    event.target.id !== "chatBotOuter" &&
    (!event.target.parentElement ||
      event.target.parentElement.id !== "chatBotOuter") &&
...
document.addEventListener("mouseup", function (event) {
  if (ismousedown && widgetInitX !== widgetFinX && widgetInitY !== widgetFinY) {
    ...
    let style =
      ...
      widgetInitX +
      "px;width: " +
      (widgetFinX - widgetInitX) +
      "px;height: " +
      (widgetFinY - widgetInitY) +
      "px;";
...
function resizeStart() {
  ...
  resizeElmnt.onDragStart = function () {
    return false;
  };
...
function initResize(e) {
  if (
    ...
    resizeElmnt &&
    dir != "" &&
    resizeElmnt.id !== "navbar" &&
    resizeElmnt.parentElement.id !== "navbar"
  ) {
    e.preventDefault();
    startX = e.clientX;
    startY = e.clientY;
...
function createInputBox(x, y, style) {
  ...
  inputbox.innerHTML =
    "<input type='text' id='inputbox-input'/><button id='inputbox-submit'>Submit</button><button id='inputbox-close'>X</button>";
...
function createInputBoxJs(x, y, element) {
  ...
  submit.addEventListener("click", function () {
    ...
    let tempHTML = getElement(element).outerHTML.replace(
      element.outerHTML,
      temp.outerHTML
    );
    vscode.postMessage({
...
function createInputBoxAttr(x, y, element) {
  ...
  inputbox.innerHTML =
    "<textarea rows='7' id='inputbox-text'>" +
    json +
    "</textarea><button id='inputbox-submit'>Submit</button><button id='inputbox-close'>X</button>";
  document.body.appendChild(inputbox);
  ...
  submit.addEventListener("click", function () {
    ...
    if (text.value) {
      ...
      let newHTML = getElement(element).outerHTML.replace(
        element.outerHTML,
        newEle.outerHTML
      );
      vscode.postMessage({
...
function dragStart(e) {
  if (
    mode == iconDic["icon-drag"] &&
    e.target.id != "navbar" &&
    e.target.parentNode.id != "navbar"
  ) {
    e.preventDefault();
    div = e.target;
...
/**
 * Generate general history
 * @param {Array} logData array of logs
 * @param {Boolean} remain whether to keep the remaining elements in the panel
...
function createHistoryPanel(logData, remain, memberRid) {
    ...
    logData.forEach((element, index) => {
      ...
      if (memberRid) {
        ...
        Create Date: ${element.createDate.slice(
          0,
          element.createDate.indexOf(".")
        )}
        </td></tr></table>`;
...
/**
 * Generate general history, Groups, and Groups Details + Editing
...
function createGroupPanel(logData) {
  ...
  for (let index = 0; index < logData.length; index++) {
    ...
    historyBlock.setAttribute("index", index);
      ...
      let elementLabels = Object.values(log.member);
      ...
      Create Date: ${log.createDate.slice(
        0,
        log.createDate.indexOf(".")
      )}
      </td></tr></table>`;
      ...
      historyBlock.addEventListener("click", function (e) {
        ...
        if (document.getElementById("detailBtn")) {
          document
            ...
            .addEventListener("click", function () {
              let elements = Object.keys(log.member);
              currGroupRid = log.rId;
              emptySidePanel();
              member = {};
...
function hstBlockEventListnerForInsert(hstBlock, element, nlp, style) {
  hstBlock.addEventListener("click", function (e) {
    ...
    confirmation.addEventListener("click", function () {
      ...
      let replaceStyle = handleTextReplace(
        style,
        'style="' + codeBlock.getAttribute("style") + '"'
      );
      codeBlock.style = replaceStyle.substring(
        'style="'.length,
        replaceStyle.length - 2
      );
  ...
  hstBlock.addEventListener("click", function (e) {
    ...
    confirmation.addEventListener("click", function () {
      ...
      for (let i = 0; i < Object.keys(element.member).length; i++) {
        remaining.push({
          label: element.member[Object.keys(element.member)[i]],
          rid: Object.keys(element.member)[i],
          code: element.codes[i],
        });
      }
      vscode.postMessage({
        type: "onInsert",
    ...
    logData.forEach((element, index) => {
      ...
      for (let i = 0; i < element.repeated.length; i++) {
        if (element.repeated[i].old !== element.repeated[i].new) {
          repeatedText +=
            " " +
            element.repeated[i].new +
            " (" +
            element.repeated[i].old +
            "?),";
        } else {
        ...
        for (let i = 0; i < hstBlocks.length; i++) {
          ...
          hstBlock.addEventListener("click", function (e) {
            ...
            if (event != "reset") {
              ...
              actionBtn.innerHTML =
                "<button id='resetBtn' >Reset</button>; <button id='undoBtn' >Undo</button>;";
...
function getCopilotText(element) {
  ...
  element.getAttributeNames().reduce((acc, name) => {
    if (
      ...
      name.toLowerCase() !== "nlp" &&
      name.toLowerCase() !== "rid" &&
      name.toLowerCase() !== "eid"
    ) {
...
function handleTextReplace(insertStyle, replaceStyle) {
  ...
  replaceStyle = replaceStyle.replace(
    replaceStyle.slice(styleStart + 1, styleEnd),
    innerText + ","
  );
  return replaceStyle;
}
...
/**
 * Creates a new element with specified classes and adds it to a parent element.
 * @param {string} tag
 * @param {HTMLElement | null} parent
 * @param {Array | string} classes Single class or array of classes
 */
